...
if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.error("Error: MIXMI_OPS_SUPABASE_URL and MIXMI_OPS_SUPABASE_KEY environment variables are required.\n" +
...
// ============================================================
server.registerTool("ops_node_overview", {
    title: "Pilot Node Overview",
    description: "Get the current status of all pilot nodes, or a specific node by name. " +
    ...
    if (!data || data.length === 0)
        return textResponse("No pilot nodes found matching that name.");
    const lines = data.map((n) => `## ${n.name}\n` +
        `- **Status:** ${n.status}\n` +
        `- **Lead:** ${n.lead_name || "unassigned"}\n` +
        `- **Participants:** ${n.participant_count}\n` +
        `- **Activation date:** ${formatDate(n.activation_date)}\n` +
        ...
        `- **Last activity:** ${formatDate(n.last_activity)}`);
...
// ============================================================
server.registerTool("ops_list_milestones", {
    title: "List Milestones",
    description: "List milestones across all workstreams, with optional filters by stream, status, or node. " +
    ...
    if (error)
        return errorResponse(error.message);
    if (!data || data.length === 0)
        return textResponse("No milestones found with those filters.");
    let filtered = data;
    if (node_name) {
        filtered = data.filter((m) => {
            const node = m.pilot_nodes;
            return node && node.name.toLowerCase().includes(node_name.toLowerCase());
        });
    }
    const statusIcon = {
    ...
    const lines = filtered.map((m) => {
        ...
        return `${statusIcon[m.status] || "📋"} **${m.title}**${nodeName}\n` +
...
// ============================================================
server.registerTool("ops_update_milestone", {
    title: "Update Milestone",
    description: "Update a milestone's status, target week, owner, or mark it complete. " +
    ...
    if (findError)
        return errorResponse(findError.message);
    if (!matches || matches.length === 0)
        return textResponse(`No milestone found matching "${title_search}".`);
    if (matches.length > 1) {
...
server.registerTool("ops_list_tasks", {
    title: "List Tasks",
    description: "List tasks across all nodes, with optional filters. " +
    ...
    if (error)
        return errorResponse(error.message);
    if (!data || data.length === 0)
        return textResponse("No tasks found. Tasks will appear as you create them.");
    let filtered = data;
    if (status)
        filtered = filtered.filter((t) => t.status === status);
    if (node_name)
        filtered = filtered.filter((t) => t.node_name?.toLowerCase().includes(node_name.toLowerCase()));
    if (assignee)
        filtered = filtered.filter((t) => t.assignee?.toLowerCase().includes(assignee.toLowerCase()));
    if (filtered.length === 0)
        return textResponse("No tasks match those filters.");
    const lines = filtered.map((t) => `- [${t.priority}] **${t.title}** — ${t.status}` +
...
server.registerTool("ops_create_task", {
    title: "Create Task",
    description: "Create a new task, optionally linked to a milestone and pilot node.",
    inputSchema: {
...
// ============================================================
server.registerTool("ops_log_engagement", {
    title: "Log Engagement",
    description: "Log an activity or engagement event for a pilot node. " +
    ...
    if (!nodes || nodes.length === 0)
        return errorResponse(`No node found matching "${node_name}".`);
    const node = nodes[0];
    ...
    if (error)
        return errorResponse(error.message);
    return textResponse(`Logged ${event} for ${node.name}: "${title}"` +
...
// ============================================================
server.registerTool("ops_activity_feed", {
    title: "Activity Feed",
    description: "Get recent engagement activity across all pilot nodes. " +
    ...
    let query = supabase
        ...
        .limit(limit);
    if (node_name)
        query = query.ilike("node_name", `%${node_name}%`);
    const { data, error } = await query;
    if (error)
        return errorResponse(error.message);
    if (!data || data.length === 0)
        return textResponse("No activity logged yet. Use ops_log_engagement to record events.");
    const lines = data.map((e) => `- **${formatDate(e.created_at)}** [${e.node_name}] ${e.event}: ${e.title}` +
...
// ============================================================
server.registerTool("ops_list_contacts", {
    title: "List Contacts",
    description: "Search or list community contacts (CRM). Filter by node, relationship stage, or search by name.",
    inputSchema: {
    ...
    if (error)
        return errorResponse(error.message);
    if (!data || data.length === 0)
        return textResponse("No contacts found matching those filters.");
    let filtered = data;
    if (node_name) {
        filtered = data.filter((c) => {
            const node = c.pilot_nodes;
            return node && node.name.toLowerCase().includes(node_name.toLowerCase());
        });
    }
    const lines = filtered.map((c) => {
        ...
        return `- **${c.name}** — ${c.role || "no role"}` +
            ...
            (c.next_followup ? ` | Follow up: ${formatDate(c.next_followup)}` : "") +
            (c.context ? `\n  ${c.context.slice(0, 150)}...` : "");
    });
...
// Record a decision
// ============================================================
server.registerTool("ops_log_decision", {
    title: "Log Decision",
    description: "Record a decision in the decisions log with context and reasoning. " +
...
// ============================================================
server.registerTool("ops_dashboard", {
    title: "Full Dashboard",
    description: "Get a comprehensive operational dashboard: all node statuses, in-progress milestones, " +
    ...
    if (recentActivity && recentActivity.length > 0) {
        for (const e of recentActivity) {
            output += `- ${formatDate(e.created_at)} [${e.node_name}] ${e.title}\n`;
        }
    }
    else {
    ...
    if (followups && followups.length > 0) {
        for (const f of followups) {
            const node = f.pilot_nodes;
            output += `- **${f.name}** — ${formatDate(f.next_followup)}` +
                (node ? ` (${node.name})` : "") + "\n";
        }
    }
    else {
