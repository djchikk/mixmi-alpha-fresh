...
// Social platform options - ordered by popularity for creators/musicians
const platformOptions = [
  ...
  { value: "twitter", label: "X", icon: <FaXTwitter size={16} /> },
...
export default function UnifiedProfileModal({ isOpen, onClose }: UnifiedProfileModalProps) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (dropdownOpen && !dropdown?.querySelector('button')?.contains(target)) {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Profile">
            ...
            <div className="space-y-3">
              ...
              {socialLinks.map((link, index) => (
                <div key={index} className="flex items-center space-x-2">
                  <div className="flex items-center px-3 py-2 bg-slate-800 border border-slate-600 rounded-md text-gray-300 flex-1 min-w-0">
          ...
          <div className="space-y-3">
            ...
            <div className="flex space-x-2">
              ...
              <div className="relative w-[100px] shrink-0">
                <button
                  type="button"
                  className="flex items-center justify-between w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded-md text-gray-300 focus:outline-none focus:ring-2 focus:ring-[#81E4F2]"
                ...
                {dropdownOpen && (
                  <div className="absolute z-10 w-[200px] mt-1 bg-slate-800 border border-slate-600 rounded-md shadow-lg">
                    <ul className="py-1 max-h-48 overflow-auto">
                      {platformOptions.map((option) => (
                        <li key={option.value}>
                          <button
                            type="button"
                            className={`flex items-center w-full px-3 py-2 text-sm hover:bg-slate-700 ${
                              platform === option.value ? "bg-slate-700 text-[#81E4F2]" : "text-gray-300"
                            }`}
                    ...
                    <div className="border-t border-slate-600 px-3 py-1 bg-slate-700/50">
                      <p className="text-xs text-gray-500 text-center">Scroll for more platforms</p>
                    </div>
                  </div>
                )}
              </div>
      ...
      {activeTab === "wallet" && (
        <div className="space-y-6">
          ...
          {/* Visibility settings */}
          <div className="space-y-2">
            <h3 className="text-sm font-medium text-gray-300">Visibility Settings</h3>
            ...
            <div className="flex items-center">
              <input
                ...
                className="h-4 w-4 bg-slate-800 border-slate-600 rounded text-[#81E4F2] focus:ring-[#81E4F2]"
                disabled={!walletAddress}
              />
              <label htmlFor="showStx" className="ml-2 text-sm text-gray-300">
            ...
            <div className="flex items-center">
              <input
                ...
                className="h-4 w-4 bg-slate-800 border-slate-600 rounded text-[#81E4F2] focus:ring-[#81E4F2]"
              />
              <label htmlFor="showBtc" className="ml-2 text-sm text-gray-300">
