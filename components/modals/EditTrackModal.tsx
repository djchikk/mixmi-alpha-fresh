...
interface EditTrackModalProps {
  isOpen: boolean;
  onClose: () => void;
  track: IPTrack; // Required - we're always editing an existing track
  onSave: () => void;
}
...
export default function EditTrackModal({
  ...
  const handleHideToggle = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showToast(data.error || 'Failed to update track visibility', 'error');
      } else {
        showToast('Track visibility updated!', 'success');
        onSave();
        onClose();
      }
    } catch (error) {
  ...
  const renderBasicInfo = () => (
    <div className="space-y-6" style={{ gap: '24px' }}>
      ...
      {/* Content Type Selection - Display Only (cannot be changed after upload) */}
      <div>
        <label className="block text-sm font-normal text-gray-300 mb-3">
          Content Type <span className="text-xs text-gray-500">(cannot be changed)</span>
        </label>
  ...
  const renderFileUploads = () => (
    <div className="space-y-6">
      ...
      {/* Audio Files - Display Only (cannot be changed after upload) */}
      <div>
        <label className="block text-lg font-semibold text-gray-200 mb-3">
          {formData.content_type === 'loop_pack' ? 'Loop Files' :
           formData.content_type === 'ep' ? 'EP Songs' : 'Audio File'}
          <span className="text-xs text-gray-500 ml-2">(cannot be changed)</span>
        </label>
        <div className="bg-slate-800/50 border border-slate-600 rounded-lg p-6 opacity-60 cursor-not-allowed">
          <div className="flex items-center gap-3">
            <div className="text-3xl">🎵</div>
            <div>
              <div className="text-white font-medium mb-1">Audio files cannot be changed</div>
              <div className="text-sm text-gray-400">
                The original {formData.content_type === 'loop_pack' ? 'loops' : formData.content_type === 'ep' ? 'songs' : 'audio file'} will remain unchanged.
              </div>
            </div>
          </div>
        </div>
  ...
  const renderStep = () => {
    ...
    // Edit modal only uses the 4-step quick upload flow
    switch (currentStep) {
      case 0:
  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* Mode toggle removed - Edit modal is always in quick mode (no attribution editing) */}
        ...
        {/* Step Indicator - Hide during authentication or delete confirmation */}
        {isAuthenticated && !showDeleteConfirm && (
          <div className="flex items-center justify-between">
        ...
        {/* Only show step title when authenticated and not in delete confirmation */}
        {isAuthenticated && !showDeleteConfirm && (
        ...
        {/* Validation Errors */}
        {validationErrors && validationErrors.length > 0 && !showDeleteConfirm && (
        ...
        {/* Navigation - Hide during authentication or delete confirmation */}
        {isAuthenticated && !showDeleteConfirm && (
