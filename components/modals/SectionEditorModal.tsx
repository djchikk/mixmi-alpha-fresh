    ...
    >
      ...
      <div className="relative w-12 h-12 overflow-hidden rounded-md border border-slate-700 flex-shrink-0">
        {hasValidImage ? (
          <SafeImage 
            ...
            onError={() => console.warn('Failed to load thumbnail image:', item[imageField])}
          />
        ) : item.type ? (
      ...
      <div className="flex-1 min-w-0">
        <h3 className="text-white font-medium text-sm truncate">
          {item.title}
          {item.id === 'store-card' && (
            <span className="ml-2 text-xs text-accent bg-accent/10 px-2 py-0.5 rounded">Store</span>
          )}
        </h3>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button 
          ...
          className="p-1 rounded-full bg-slate-700 hover:bg-slate-600"
          aria-label={item.id === 'store-card' ? "Hide" : "Delete"}
          title={item.id === 'store-card' ? "Hide Store Card" : "Delete Item"}
        >
          {item.id === 'store-card' ? (
            <svg 
              xmlns="http://www.w3.org/2000/svg" 
