...
export default function IPTrackModal({
  ...
  const renderBasicInfo = () => (
    <div className="space-y-6">
      {/* TEMPORARY: Wallet Override for Database Cleanup */}
      <div className="bg-orange-900/20 border border-orange-500/50 rounded-lg p-4">
        <label className="block text-sm font-medium text-orange-300 mb-2">
          🔧 ADMIN: Wallet Override (Database Cleanup Only)
        </label>
        <input
          type="text"
          value={formData.uploader_wallet_override || ''}
          onChange={(e) => handleInputChange('uploader_wallet_override', e.target.value)}
          className="w-full py-1.5 px-3 bg-orange-900/30 border border-orange-600 rounded-md text-white placeholder-orange-300/60 focus:outline-none focus:border-orange-400"
          placeholder="SP1234... (mainnet wallet address for re-registration)"
        />
        <p className="text-xs text-orange-300/80 mt-1">
          Leave empty for normal operation. Fill with SP wallet address when re-registering alpha content.
        </p>
      </div>

      {/* Content Type Selection - Segmented Control */}
      <div>
        <label className="block text-sm font-normal text-gray-300 mb-2">Content Type</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2">
          <label className="block text-sm font-normal text-gray-300 mb-2">Track Title</label>
          <input
            type="text"
        ...
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full py-1.5 px-3 bg-slate-800 border border-slate-600 rounded-md text-white focus:outline-none focus:border-[#81E4F2]"
            >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-normal text-gray-300 mb-2">
            ...
            {!(formData.content_type === 'loop' && !isQuickUpload) && <span className="text-gray-500"> (optional)</span>}
          </label>
          <input
            type="number"
            value={formData.bpm || ''}
  ...
  const renderFileUploads = () => (
    <div className="space-y-6">
      {/* Cover Image */}
      <div>
        <label className="block text-sm font-normal text-gray-300 mb-2">Cover Artwork <span className="text-gray-500">(optional)</span></label>
      ...
      {/* Audio Upload */}
      <div>
        <label className="block text-sm font-normal text-gray-300 mb-2">Audio File</label>
      ...
      <div className="space-y-6">
        ...
        {!track && currentStep === 0 && (
          <div className="text-center mb-4">
            {isQuickUpload ? (
              <div className="bg-slate-800/50 p-3 rounded-lg border border-[#81E4F2]">
                <p className="text-sm text-gray-300">
                  <span className="text-[#81E4F2] font-medium">⚡ Quick Upload:</span> Perfect for solo creators. 
        ...
        <div className="text-center">
          <h3 className="text-2xl font-semibold text-white">{getStepsArray[currentStep]}</h3>
          {currentStep === 0 && (
            <p className="text-xs font-normal text-gray-400 mt-1">
              {isQuickUpload 
        ...
        <div className="flex justify-center pt-8 border-t border-slate-700">
          <div className="flex gap-6">
            <button
              ...
              className="px-4 py-1.5 bg-slate-800 text-gray-400 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-700 hover:text-white transition-colors font-medium"
            >
              Previous
            </button>
