...
export default function SpotlightSection() {
  ...
  const handleSave = async (item: SpotlightItem) => {
    ...
    if (editingItem) {
      console.log('✏️ Calling updateSpotlightItem for:', item.id);
      await updateSpotlightItem(item.id, item);
    } else {
      console.log('➕ Calling addSpotlightItem for:', item.id);
      await addSpotlightItem(item);
    }
