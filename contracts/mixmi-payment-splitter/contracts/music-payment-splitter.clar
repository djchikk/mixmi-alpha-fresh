...
(define-public (split-track-payment
  (total-price uint)
  (composition-splits (list 10 {wallet: principal, percentage: uint}))
  (recording-splits (list 10 {wallet: principal, percentage: uint})))
...
(define-private (distribute-splits
  (pool-amount uint)
  (splits (list 10 {wallet: principal, percentage: uint})))
...
(define-read-only (validate-percentages (splits (list 10 {wallet: principal, percentage: uint})))
  (let
    (
